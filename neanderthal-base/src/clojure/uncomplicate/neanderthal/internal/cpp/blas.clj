...
(defmacro tb-sv [blas method ptr a b]
  `(let [reg# (region ~a)
     ...
     (throw (dragan-says-ex "TODO cblas_?tbsv seems to be broken, or I cna't figure out how to properly use tb matrices."))
     #_(dotimes [j# (ncols ~b)]
...
(defmacro gd-sv [blas method ptr a b]
  `(let [layout# (.layout (navigator ~a))
         ...
         stride-col-b# (if (.isColumnMajor nav-b#) 1 (stride ~b))]
     (dotimes [j# (ncols ~b)]
       (. ~blas ~method ~(:row blas-layout) ~(:lower blas-uplo) ~(:no-trans blas-transpose)
